  ...
  private void authenticateToDevHub() {
    _.withCredentials( [ _.file( credentialsId: _.env.JWT_CRED_ID_DH, variable: 'jwt_key_file') ] ) {
       ...
       // _.sh returnStatus: true, script: "cp ${_.jwt_key_file} ./server.key"
        ...
        def rc = _.bat returnStatus: true, script: "\"${toolbelt}\" force:auth:jwt:grant --clientid ${_.env.CONNECTED_APP_CONSUMER_KEY} --username ${_.env.HUB_ORG} --jwtkeyfile server.key --setdefaultdevhubusername --instanceurl ${_.env.SFDC_HOST_DH}"
